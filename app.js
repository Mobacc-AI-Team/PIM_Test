...
function setStartSelection(adminCode) {
  activeAdmin = adminCode;
  adminSelect.value = adminCode;
  setTheme(adminCode);
  document.querySelectorAll("[data-admin-card]").forEach((card) => {
    card.classList.toggle("active", card.dataset.adminCard === adminCode);
  });
  startSelected.textContent = `Geselecteerd: ${adminCode} - ${administrations[adminCode].name}`;
}
...
document.querySelectorAll("[data-admin-card]").forEach((card) => {
  card.addEventListener("click", () => {
    setStartSelection(card.dataset.adminCard);
  });
});
